WITH
campaigns_performance_networks AS (
  SELECT
    ...
    COALESCE(SUM(f_creative.num_impression), 0) AS total_impressions,

    COALESCE(SUM(f_creative.num_first_install), 0)
      + COALESCE(SUM(f_creative.num_reinstall), 0)
      + COALESCE(SUM(f_creative.num_reopen), 0) AS total_installs,

    COALESCE(SUM(f_creative.cost_usd), 0) AS total_cost,
    COALESCE(SUM(f_creative.num_click), 0) AS total_clicks,
    ...
    COALESCE(
      SUM(
        CASE
          WHEN DATE_DIFF(
            DATE(f_creative.activity_date),
            DATE(f_creative.install_date),
            DAY
          ) = 1
      ...
      COALESCE(
        SUM(
          CASE
            ...
            THEN COALESCE(f_creative.num_first_install, 0)
  ...
  WHERE
    f_creative.install_date BETWEEN "PARAM-START-DATE" AND "PARAM-END-DATE"
    AND f_creative.acquisition_category = "Paid"
    AND f_creative.network IN ("network_a", "network_b")
    AND f_creative.campaign_name LIKE CONCAT('%', "PARAM-TEST-BATCH-NUMBER", '%')
...
campaigns_performance_social AS (
  SELECT
    f_creative.device_class AS device_group,
    f_creative.channel_name AS network,
    d_product.product_short_name AS product_name,
    ...
    COALESCE(SUM(f_creative.num_creative_impression), 0) AS total_impressions,

    COALESCE(SUM(f_creative.num_first_install), 0)
      + COALESCE(SUM(f_creative.num_reinstall), 0)
      + COALESCE(SUM(f_creative.num_reopen), 0) AS total_installs,

    COALESCE(SUM(f_creative.creative_cost_usd), 0) AS total_cost,
    COALESCE(SUM(f_creative.num_creative_click), 0) AS total_clicks,

    DATE(
      MIN(
        CASE
          WHEN f_creative.num_creative_impression > 0
          THEN CAST(f_creative.activity_date AS TIMESTAMP)
          ELSE NULL
        END
      )
  ...
  FROM
    `project.dataset.social_creative_performance_daily` AS f_creative
  ...
  WHERE
    f_creative.install_date BETWEEN "PARAM-START-DATE" AND "PARAM-END-DATE"
    AND f_creative.acquisition_category = "Paid"
    AND f_creative.channel_name = "network_c"
    AND f_creative.campaign_name LIKE CONCAT('%', "PARAM-TEST-BATCH-NUMBER", '%')
    AND f_creative.campaign_acquisition_type = "User Acquisition"
    AND f_creative.campaign_category IN ("Test", "PlayTest")
    AND (
      f_creative.campaign_test_type = "Creative"
      OR f_creative.campaign_test_type = "N/A"
    )
    AND f_creative.device_class = "Android"
...
campaigns_performance AS (
  SELECT * FROM campaigns_performance_networks

  UNION ALL

  SELECT * FROM campaigns_performance_social
)
